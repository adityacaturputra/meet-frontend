...
const layoutCandidates = Array.from({
  ...
  .reduce(
    (prev, curr) => ({ ...prev, [curr.count]: curr.candidates }),
    {},
  );
...
const maxRowsColumns = (width, height) => ({
  ...
  maxRows: Math.max(
    1,
    Math.floor(height / (minCellHeight + cellOffset * 2)),
  ),
});
...
export function getVideoLayout(rootWidth, rootHeight, count) {
  ...
  const verticalMargin =
    (rootHeight - cellBoxHeight * row) / 2 + cellOffset;
  ...
  for (let i = 0; i < row; i += 1) {
    for (let j = 0; j < column; j += 1) {
      const leftMargin =
        i !== row - 1 ? horizontalMargin : lastRowMargin;
